        ...
        <div className="w-full px-4 bg-[#FAF9F6] md:px-11 flex justify-between items-center gap-44 sm:gap-24">
          ...
          <div className="hidden md:flex items-center gap-4 text-base font-medium">
            {navigationItems.map((item, index) => (
              <Texthover
                text={item.name}
                className="opacity-85  hover:opacity-100 font-normal hover:scale-105 transition-all duration-200 ease-in-out hover:-translate-y-1 active:translate-y-0 active:scale-90"
                key={index}
                href={item.href}
              ></Texthover>
            ))}
            ...
            >
              {navigationItems.map((item, index) => (
                <motion.a
                  ...
                  key={index}
                >
                  <Texthover text={item.name} href={item.href}></Texthover>
                </motion.a>
              ))}
            </motion.div>
          )}
...
function MenuToggle({ isMenuOpen, setMenuOpen }: MenuToggleProps) {
  return (
    <button
      className={cn(
        "md:hidden group inline-flex w-12 h-12 text-center items-center justify-center transition rounded-lg text-black"
      )}
      onClick={() => setMenuOpen(!isMenuOpen)}
    >
      <svg
        className="w-6 h-6 fillCurrent pointer-events-none"
        viewBox="0 0 16 16"
      ...
      >
        <rect
          className={cn(
            ...
            isMenuOpen ? "rotate-[315deg] [x:0] [y:7]" : ""
          )}
          y="2"
          x="7"
          width="9"
        ...
        <rect
          className={cn(
            "origin-center transition-all duration-300 ease-[cubic-bezier(.5,.85,.25,1.1)]",
            isMenuOpen ? "[y:7] [x:0] -rotate-[225deg]" : ""
          )}
          y="12"
          width="9"
          height="1.5"
